 ...
 *
 * <p>Giúp đính kèm thêm các siêu dữ liệu quan trọng như vai trò (`roles`), quyền hạn (`permissions`),
 * email và tên hiển thị đầy đủ của người dùng đăng nhập vào Access Token.
 *
 * <p>Tại sao cần chèn trực tiếp các claims này vào Access Token?
 * - Giúp API Gateway và các Service nội bộ có thể tự động parse và phân quyền ngoại tuyến (offline auth checking)
 *   mà không cần phải tạo cuộc gọi mạng ngược lại SSO Server để truy vấn thông tin user.
 * - Tiết kiệm CPU, giảm thiểu độ trễ (latency) của toàn hệ thống phân tán.
 *
    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer() {
        return context -> {
            if ("access_token".equals(context.getTokenType().getValue())) {
                log.info("Thực hiện tùy biến chèn custom claims vào Access Token cho client {}", 
                ...
                if (principal instanceof SsoUserDetails ssoUserDetails) {
                    ...
                    Set<String> permissions = ssoUserDetails.getAuthorities().stream()
                            .map(GrantedAuthority::getAuthority)
                            .filter(auth -> !auth.startsWith("ROLE_")) // Lọc bỏ role prefix để lấy riêng permissions
                            .collect(Collectors.toSet());
                    context.getClaims().claim("permissions", permissions);
                    
                    log.debug("Đã inject claims thành công cho user: {}, Roles: {}, Permissions: {}", 
