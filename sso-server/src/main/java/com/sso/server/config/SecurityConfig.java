 ...
 *
 * <p>Định nghĩa chuỗi bộ lọc bảo mật mặc định (Default Security Filter Chain) cho
 * các tương tác người dùng trực tiếp như màn hình Đăng nhập (Form Login), Logout,
 * và xác thực tài khoản dựa trên CSDL PostgreSQL.
 *
...
public class SecurityConfig {
    ...
    public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        http
            .authorizeHttpRequests(authorize -> authorize
                .requestMatchers("/actuator/**").permitAll() // Cho phép truy cập metrics giám sát công khai
                .anyRequest().authenticated()
            )
            .formLogin(formLogin -> formLogin
                .permitAll() // Sử dụng màn hình login mặc định của Spring Security
            )
     ...
     *
     * <p>Mật khẩu của người dùng bắt buộc phải được băm (hashing) trước khi lưu trữ
     * vào database để đảm bảo an toàn, phòng ngừa rò rỉ dữ liệu.
     *
